...
module Oroshi
  class InvoiceNotificationMailerTest < ActionMailer::TestCase
    test 'invoice_notification renders the headers' do
      ...
      expected_from = Setting.find_by(name: 'oroshi_company_settings')&.settings&.dig('mail') || ENV.fetch('MAIL_SENDER',
                                                                                                           nil)
