...
class ServiceConfiguration < ApplicationRecord
  ...
  SERVICES = {
    google_cloud_storage: {
      ...
      required_credentials: %w[project bucket credentials_path]
    },
    hetzner_object_storage: {
      name: 'Hetzner Object Storage',
      description: 'Store files on Hetzner S3-compatible storage',
      required_credentials: %w[access_key secret_key endpoint bucket]
    },
    aws_s3: {
      name: 'AWS S3',
      description: 'Store files on Amazon S3',
      required_credentials: %w[access_key_id secret_access_key region bucket]
    },
    sendgrid: {
      name: 'SendGrid',
      description: 'Send emails via SendGrid',
      required_credentials: %w[api_username api_password mail_sender]
    },
    resend: {
      name: 'Resend',
    ...
    smtp: {
      ...
      required_credentials: %w[host port username password]
    },
    rakuten: {
      name: 'Rakuten Ichiba',
      description: 'Rakuten marketplace integration',
      required_credentials: %w[api_key service_secret license_key]
    },
    yahoo_v2: {
      name: 'Yahoo Shopping (V2)',
      description: 'Yahoo Shopping API V2 integration',
      required_credentials: %w[client_id client_secret seller_id]
    },
    yahoo_v1: {
      name: 'Yahoo Shopping (V1 - Legacy)',
      description: 'Yahoo Shopping API V1 (legacy support)',
      required_credentials: %w[secret client_id]
    },
    infomart: {
      name: 'Infomart',
      description: 'Infomart B2B platform integration',
      required_credentials: %w[login password]
    }
  }.freeze
