...
module Oroshi
  class InvoiceJobTest < ActiveJob::TestCase
    setup do
      @message = create(:message)
      @invoice = instance_double('Oroshi::Invoice', id: 1, start_date: Date.new(2025, 10, 1),
                                                    end_date: Date.new(2025, 10, 31), invoice_layout: 'standard', invoice_date: Date.new(2025, 10, 31))
      ...
      @join = instance_double('InvoiceSupplierOrganization', supplier_organization: @supplier_org,
    ...
    test 'finds the invoice and message' do
      ...
      expect(@message).to receive(:update).with(state: true,
                                                message: "\u4F9B\u7D66\u6599\u4ED5\u5207\u308A\u66F8\u4F5C\u6210\u5B8C\u4E86")
