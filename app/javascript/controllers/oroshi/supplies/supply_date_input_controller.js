...
export default class extends Controller {
  ...
  connect() {
    ...
    this.supplyTypeVariation = form?.dataset?.supplyTypeVariation || "";
    this.supplyType = form?.dataset?.supplyType || "";

    if (!this.supplierColumn || !this.targetFrame) return;
  ...
  updateLineSubtotal() {
    ...
    const subtotalColumn = this.targetFrame.querySelector(".subtotal-column");
    const subtotalSpan = subtotalColumn?.querySelectorAll("span.supply-type-variation-line-subtotal")[index];
  ...
  updateSupplierSupplyTypeVariationSubtotal() {
    if (!this.supplierColumn || !this.supplyTypeVariation) return;
    ...
    if (!supplierTypeVariationSpan) return;
  ...
  updateSupplyTypeVariationSubtotal() {
    if (!this.targetFrame || !this.supplyTypeVariation) return;
    ...
    if (!supplyTypeVariationSpan) return;
  ...
  updateSupplyTypeSubtotal() {
    if (!this.targetFrame || !this.supplyType) return;
