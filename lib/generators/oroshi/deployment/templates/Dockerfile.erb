...
# GCS credentials should be mounted at runtime or provided via environment variables
# See .env.example for GOOGLE_APPLICATION_CREDENTIALS configuration
RUN mkdir -p config/gcloud

# Precompile bootsnap code for faster boot times
RUN bundle exec bootsnap precompile app/ lib/

# Build CSS with dartsass (doesn't need secret_key_base)
...
RUN groupadd --system --gid 1000 rails && \
    ...
    mkdir -p log storage tmp config/gcloud && \
    chown -R rails:rails db log storage tmp config/gcloud
USER 1000:1000
